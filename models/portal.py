...
class CustomerPortal(CustomerPortal):
    ...
    #
    def _invoice_get_page_view_values(self, order, access_token, **kwargs):
        values = {
            'page_name': 'waybills',
            'order': order,
        }
        return self._get_page_view_values(order, access_token, values, 'my_invoices_history', False, **kwargs)
    ...
    def portal_my_orders(self, page=1, date_begin=None, date_end=None, sortby=None, **kw):
        ...
        archive_groups = self._get_archive_groups('orders', domain) if values.get('my_details') else []
        if date_begin and date_end:
            domain += [('create_date', '>', date_begin), ('create_date', '<=', date_end)]

        # count for pager
        order_count = Orders.search_count(domain)
        # pager
        pager = portal_pager(
            url="/my/waybills",
        ...
        orders = Orders.search(domain, order=sort_order, limit=self._items_per_page, offset=pager['offset'])
        ...
        values.update({
            ...
            'default_url': '/my/waybills',
            'searchbar_sortings': searchbar_sortings,
            'sortby': sortby,
        })
        return request.render("fms.portal_my_orders", values)

    @http.route(['/my/waybills/<int:order_id>'], type='http', auth="public", website=True)
    def portal_order_page(self, order_id, report_type=None, access_token=None, message=False, download=False, **kw):
        try:
            order_sudo = self._document_check_access('orders', order_id, access_token=access_token)
    ...
    @http.route(['/my/waybills/<int:order_id>/accept'], type='json', auth="public", website=True)
    def portal_quote_accept(self, order_id, access_token=None, name=None, signature=None):
        ...
        _message_post_helper(
            'orders', order_sudo.id, _('Order signed by %s') % (name,),
    ...
    @http.route(['/my/waybills/<int:order_id>/decline'], type='http', auth="public", methods=['POST'], website=True)
    def decline(self, order_id, access_token=None, **post):
        ...
        if order_sudo.has_to_be_signed() and message:
            ...
            _message_post_helper('orders', order_id, message, **{'token': access_token} if access_token else {})
        else:
    ...
    @http.route(['/my/waybills/<int:order_id>/transaction/'], type='json', auth="public", website=True)
    def payment_transaction_token(self, acquirer_id, order_id, save_token=False, access_token=None, **kwargs):
        ...
        order = request.env['orders'].sudo().browse(order_id)
    ...
    @http.route('/my/waybills/<int:order_id>/transaction/token', type='http', auth='public', website=True)
    def payment_token(self, order_id, pm_id=None, **kwargs):

        order = request.env['orders'].sudo().browse(order_id)
        if not order:
            return request.redirect("/my/waybills")
